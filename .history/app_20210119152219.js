...
app.get('/download', (req, res) => {
    try {
        ...
        var dlfilepath = dir + req.query.name;
        (async () => {
            try {
                var ress = await isFileExisted(dlfilepath);
                console.log("yyyyyyy===", ress)
                if (ress) {
                    fs.createReadStream(dlfilepath).pipe(res);
                } else {
                    fs.createReadStream(dir + "404.png").pipe(res);
                }
            } catch (error) {
                console.log(error);
            }
